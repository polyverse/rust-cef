...
use syn::{
    Attribute, Data, DataEnum, DataStruct, DeriveInput, Error as SynError, Fields, Ident,
    Index, Meta, NestedMeta, Variant,
};
    ...
    let field_extractions_result: Result<Vec<(TokenStream2, TokenStream2)>, TokenStream2> = variant
        ...
        .map(
            |(index, f)| -> Result<(TokenStream2, TokenStream2), TokenStream2> {
                ...
                let (field_prefix, fieldid, field_name_from_id) = match &f.ident {
                    Some(id) => (quote! {#id:}, format_ident!("_{}", id), FieldNameFromId::Allowed),
                    None => (quote! {}, format_ident!("_index{}", index), FieldNameFromId::NotAllowed),
                };
