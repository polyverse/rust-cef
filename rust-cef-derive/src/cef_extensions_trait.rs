...
const CEF_EXT_VALUES_USAGE: &str = "'cef_ext_values' macro expects extension values to be listed in the following syntax: #[cef_ext_values(extensionKey1 = \"value1\", extensionKey2 = \"value2\", ...)] ";
...
const CEF_EXT_OPTIONAL_GOBBLE_USAGE: &str = "'cef_ext_optional_gobble' macro must supply no arguments and appear by itself to inform CefExtensions derivation to gobble any keys generated by that Optional<field> type's CefExtensions implementation. #[cef_ext_optional_gobble]";
const CEF_EXT_OPTIONAL_FIELD_USAGE: &str = "'cef_ext_optional_field' macro may optionally supply one argument which is the custom extension key name to use. If no arguments are supplied, the field's name is used. #[cef_ext_optional_field(rename)]";
const CEF_EXT_GOBBLE_KV_ITERATOR_USAGE: &str = "'cef_ext_gobble_kv_iterator' macro must supply no arguments and appear by itself to inform CefExtensions derivation to gobble key-value pairs generated by the field's iterator. Field must be an iterator over tuples (k: ToString, v: ToString). #[cef_ext_gobble_kv_iterator]";
const CEF_EXT_OPTIONAL_GOBBLE_KV_ITERATOR_USAGE: &str = "'cef_ext_optional_gobble_kv_iterator' macro must supply no arguments and appear by itself to inform CefExtensions derivation to Option'ally gobble key-value pairs generated by the field's iterator. Field must be an iterator over tuples (k: ToString, v: ToString). #[cef_ext_optional_gobble_kv_iterator]";
...
fn extensions_from_child_item(item: &DeriveInput) -> TokenStream2 {
    let fixed_values = top_level_cef_ext_values(&item.attrs);

    // Is the Item a struct or enum?
    let field_values = match &item.data {
...
fn extensions_from_child_struct(s: &DataStruct) -> TokenStream2 {
    ...
    let field_extension_exprs_result: OptionalCollectedCompileResult = s
        .fields
        .iter()
        .enumerate()
        .map(|(index, field)| {
    ...
    let field_extension_exprs: Vec<TokenStream2> = match field_extension_exprs_result {
        Err(e) => return e,
...
fn destructure_and_match_variant(variant: &Variant) -> OptionalCompileResult {
    ...
    let fixed_values = top_level_cef_ext_values(&variant.attrs);
...
fn field_extraction(
    ...
    let values_for_field_result: CollectedCompileResult = attrs.iter()
        .filter(|attr| attr.path.is_ident("cef_ext_gobble") || attr.path.is_ident("cef_ext_optional_gobble") || attr.path.is_ident("cef_ext_field") || attr.path.is_ident("cef_ext_optional_field") || attr.path.is_ident("cef_ext_gobble_kv_iterator") || attr.path.is_ident("cef_ext_optional_gobble_kv_iterator") || attr.path.is_ident("cef_ext_values"))
        .map(|attr| {
            let (usage_message, value_type, optional) = match attr.path.get_ident() {
                ...
                Some(ident) => match ident.to_string().as_str() {
                    "cef_ext_gobble" => (CEF_EXT_GOBBLE_USAGE.to_owned(), FieldValueType::GobbleTrait, false),
                    "cef_ext_optional_gobble" => (CEF_EXT_OPTIONAL_GOBBLE_USAGE.to_owned(), FieldValueType::GobbleTrait, true),
                    "cef_ext_field" => (CEF_EXT_FIELD_USAGE.to_owned(), FieldValueType::DisplayTrait, false),
                    "cef_ext_optional_field" => (CEF_EXT_OPTIONAL_FIELD_USAGE.to_owned(), FieldValueType::DisplayTrait, true),
                    "cef_ext_gobble_kv_iterator" => (CEF_EXT_GOBBLE_KV_ITERATOR_USAGE.to_owned(), FieldValueType::KvIterator, false),
                    "cef_ext_optional_gobble_kv_iterator" => (CEF_EXT_OPTIONAL_GOBBLE_KV_ITERATOR_USAGE.to_owned(), FieldValueType::KvIterator, true),
                    "cef_ext_values" => return Err(SynError::new(attr.span(), CEF_EXT_VALUES_APPLICABLE).to_compile_error()),
                    _ => return Err(SynError::new(attr.span(), "attribute ident not understood, and the internal filter should have protected you from it. This is a bug in rust-cef-derive crate.".to_owned()).to_compile_error()),
                }
            };
            ...
            match parse_attrs_to_path(&attr, usage_message.as_str()) {
                Ok(None) => match &field_identity {
                    FieldIdentity::Ident(fieldid) => match value_type {
                        FieldValueType::GobbleTrait | FieldValueType::KvIterator => Ok(field_value(fieldid.to_string().as_str(), fieldid, &value_type, prefix_self, optional)),
                        FieldValueType::DisplayTrait if FieldNameFromId::Allowed == field_name_from_id => Ok(field_value(fieldid.to_string().as_str(), fieldid, &value_type, prefix_self, optional)),
                    ...
                    FieldIdentity::Index(index) => match value_type {
                        FieldValueType::GobbleTrait | FieldValueType::KvIterator => Ok(field_value("ignored", index, &value_type, prefix_self, optional)),
                ...
                Ok(Some(newfield)) => match &field_identity {
                    FieldIdentity::Ident(fieldid) => Ok(field_value(newfield.as_str(), fieldid, &value_type, prefix_self, optional)),
                    FieldIdentity::Index(index) => match value_type {
                        FieldValueType::GobbleTrait | FieldValueType::KvIterator => Ok(field_value("ignored", index, &value_type, prefix_self, optional)),
                        FieldValueType::DisplayTrait => Ok(field_value(newfield.as_str(), index, &value_type, prefix_self, optional)),
                    },
                },
                Err(e) => return Err(e),
            }
...
/// ```
///
fn top_level_cef_ext_values(attrs: &Vec<Attribute>) -> Vec<TokenStream2> {
    let mut retval = vec![];
    for attr in attrs {
            ...
            Some(attr_name) => {
                if attr_name.starts_with("cef_ext_") {
                    retval.push(
                        SynError::new(
                            attr.span(),
                            format!("'{}': Not supported on Struct or Enums.", attr_name),
                        )
                        .to_compile_error(),
                    )
                }
